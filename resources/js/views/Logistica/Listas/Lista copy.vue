      ...
      <div class="row">
        <div class="col-12 col-lg-12 col-xxl-12 d-flex">
          <div class="card flex-fill">
              ...
              <div class="row">
                ...
                <div class="mb-3 col-sm-4">
                  ...
                  <v-select disabled input-id="encabezado_profesor_id" v-model="selectEncabezadoProfesor" label="text"
                    placeholder="Seleccione un profesor" no-options="Seleccione una opción" :loading="selectLoading"
                ...
                <div class="mb-3 col-sm-4">
                  ...
                  <v-select disabled input-id="encabezado_materia_id" v-model="selectEncabezadomateria" label="text"
                    placeholder="Seleccione una materia" no-options="Seleccione una opción" :loading="selectLoading"
                    no-results="No se encontraron resultados" />
                </div>
                ...
                <div class="mb-3 col-sm-4">
                  ...
                  <v-select disabled input-id="encabezado_semestre_id" v-model="selectEncabezadosemestre" label="text"
                    placeholder="Seleccione un semestre" no-options="Seleccione una opción" :loading="selectLoading"
                    no-results="No se encontraron resultados" />
                </div>
                ...
                <div class="mb-3 col-sm-4">
                  ...
                  <v-select disabled input-id="encabezado_grupo_id" v-model="selectEncabezadogrupo" label="text"
                    placeholder="Seleccione un grupo" no-options="Seleccione una opción" :loading="selectLoading"
                    no-results="No se encontraron resultados" />
                </div>
                ...
                <div class="mb-3 col-sm-4">
                  ...
                  <v-select disabled input-id="encabezado_carrera_id" v-model="selectEncabezadocarrera" label="text"
                    placeholder="Seleccione una carrera" no-options="Seleccione una opción" :loading="selectLoading"
                    no-results="No se encontraron resultados" />
                </div>
                ...
                <div class="mb-3 col-sm-4">
                  ...
                  <v-select disabled input-id="encabezado_ano_id" v-model="selectEncabezadoano" label="text"
                    placeholder="Seleccione un ano" no-options="Seleccione una opción" :loading="selectLoading"
                    no-results="No se encontraron resultados" />
                </div>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-sm-4 col-md-3">
                  <v-select v-model="selectSelectedParcial" :options="selectOptionsParciales" label="text"
                    ...
                    @search="selectFetchOptionsParciales" :reduce="option => option"
            ...
            <EasyDataTable :headers="headers" :items="Alumnos" :loading="cargando" :rows-per-page="5"
              table-class="table table-hover my-0">

              <!-- 🎯 Columna editable: Nombre -->
              <!-- <template #item-name="{ id, name }">
                <input class="form-control form-control-sm" type="text" v-model="getPermisoById(id).name"
                  @blur="guardarNombre(id)" />
              </template> -->

              <!-- 🎯 Columna de acciones personalizada -->
              <template #item-action="{ id, name }">
                <div class="btn-group">
                  <button class="btn btn-sm btn-outline-danger" @click="eliminarAlumno(id, name)">
                    Eliminar
                  </button>
                </div>
              </template>
  ...
  methods: {
    async consultar() {
      try {
        ...
        const { profesor, materia, semestre, grupo, carrera, año } = res.data;
        ...
        if (error.response && error.response.status === 404) {
          ...
          this.$router.replace({ name: 'seguimiento' }); // Redirige a la lista
        } else {
          console.error(error);
        }
      }
    ...
    async consultarLista(filtro = '') {
      this.cargando = true;
      try {
        const res = await api.get(`/Estudiuante/Lista/${this.$route.params.id}`, {
    ...
    editar(id) {
      this.errores = {}; // 🔄 Limpia los errores
      const encontrado = this.Alumnos.find(p => p.id === id);
      if (encontrado) {
